...
from ehk.common.plotting import (
    plt_figure,
    setup_paper_params,
...
from experiments.paper.paths import MECHANISM_OUTPUT
from experiments.workspaces import workspace_dir
from experiments.theory_guided.social_force_probe.scenarios import SCENARIOS
from ehk.common.plotting.landscape import save_landscape_figure
from experiments.theory_guided.social_force_probe.analyze import (
    LANDSCAPE_CHOICES,
    LandscapeKind,
...
from experiments.paper.mechanism_cases.plot_supplement import (
    ParsedRecord,
    get_basic_data,
...
if __name__ == "__main__":
  ...
  if args.version == 0:
    ...
    rec_baseline = RawSimulationRecord(
        str(workspace_dir("local_mech")), cfg.all_scenarios_mech[0]
    )
    with rec_baseline:
    ...
    save_landscape_figure(
        fig, MECHANISM_OUTPUT.resolve() / "figures" / "f_mech_map_baseline"
    )
  ...
  rec_baseline_pbs = RawSimulationRecord(
      str(workspace_dir("local_mech")), cfg.all_scenarios_mech[1]
  )
  rec_baseline_sbp = RawSimulationRecord(
      str(workspace_dir("local_mech")), cfg.all_scenarios_mech[2]
  )
  ...
  save_landscape_figure(
      fig, MECHANISM_OUTPUT.resolve() / "figures" / "f_mech_map_baseline_cmp"
  )
