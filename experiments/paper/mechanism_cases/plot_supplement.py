...
from ehk.common.numerics import estimate_force_field_kde
...
from ehk.micro.observables import c

if TYPE_CHECKING:
  from smp_bindings import RawSimulationRecord

CACHE_FILE_PATH = MECHANISM_OUTPUT.resolve() / "derived" / "basic_data_cache.pkl"
PLOT_RES = 100
...
if __name__ == "__main__":
  ...
  recs = [
      RawSimulationRecord(str(workspace_dir("local_mech")), d)
  ...
  plot_group(
      ...
      str(MECHANISM_OUTPUT.resolve() / "figures" / "f_supp_mech_map_g1"),
  ...
  plot_group(
      ...
      5,
      [],
      str(MECHANISM_OUTPUT.resolve() / "figures" / "f_supp_mech_map_g2"),
      landscape=args.landscape,
  )
