...
from experiments.theory_guided.social_force_probe.scenarios import (
    INPUT_DIR,
    OUTPUT_DIR,
    PROBE_BINARY,
    NORMALIZED_TIMES,
...
from experiments.theory_guided.social_force_probe.plot import plot_results
from ehk.common.plotting.landscape import potential_from_force
...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument(
      "--workspace",
      type=Path,
      default=INPUT_DIR.resolve(),
  )
  parser.add_argument(
      "--probe-binary",
      type=Path,
      default=PROBE_BINARY.resolve(),
  )
  parser.add_argument(
      "--output-dir",
      type=Path,
      default=OUTPUT_DIR.resolve(),
  )
...
def main() -> None:
  ...
  if not args.no_plot:
    figure_paths = plot_results(
        result_paths,
        args.output_dir.resolve() / "figures" / "counterfactual_probe_landscape",
    )
